...
def download_rfc():
    ...
    while True: 
        ...
        while (j<8278):
            temp_time=time.time()
            status,dest_server=rfc_list.rfc_search(j)
            dest_port=''
#        print(peer_list)
            if status ==False:
                j=j+1
                ...
                if (dest_server in server_host) or (server_host in dest_server):
                    #print("skipping this file RFC_",j)                    
                    j=j+1 
                    continue
                for i in range(len(peer_list)):
                ...
                file_name='rfc'+str(j)+'.txt' 
                download_file = open(file_name,"wb")
                while temp_data:
                    download_file.write(temp_data)
                    temp_data = download.recv(9200)
                #print ("Download Completed for RFC_",j)
                j=j+1
                download.close()
                cumilative_time=cumilative_time+(time.time()-temp_time)
