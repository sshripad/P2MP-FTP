# -*- coding: utf-8 -*-
"""
Created on Tue Oct 17 23:54:21 2017

@author: SHRIHARSHA SHRIPADARAJ &&& JASPREET KAUR
"""
'''
...
class node(object):
    def __init__(self,data):
        self.data=data
        self.next=None
class rfclinkedlist(object):                                   #Linked List to store RFC Index
    def __init__(self):
        self.head=None
    ...
    def ttlupdate(self,value):                                    #for TTL update
        cur=self.head
        while cur!=None:
            cur.data[3]-=value
            cur=cur.next
...
def download_rfc():
    ...
    while True: 
        temp_time=time.time()
        file_name=int(raw_input("Enter the RFC number of the RFC you want to download"))
        temp_time=time.time()
        status,dest_server=rfc_list.rfc_search(file_name)
        dest_port=''
        if status ==False:
            print("We could not find the RFC which you are looking for")
            flag=False
        else:
            ...
            file_name='rfc'+str(file_name)+'.txt' 
            download_file = open(file_name,"wb")
            while temp_data:
                download_file.write(temp_data)
                temp_data = download.recv(9200)
            print ("Download Completed for RFC_",file_name)
            download.close()
            print((time.time()-temp_time))
            
        if flag==False:
            yesno= raw_input(" Do you want to try again?\n Press 'Y'  ---------> To try again\nPress 'N'  -------->  To exit")
            if yesno=='N':
                break
        else:
            yesno= raw_input(" Do you want to download another file?\n Press 'Y'  ---------> To download\nPress 'N'  -------->  To exit")
            if yesno=='N':
                break
    return()
...
def createRFC_list():
    ...
    global server_host
    global filepath
    print("Please enter the details of local RFCs ")
    i=8199
    while i<8278:#or 'QUIT' or 'exit' or 'EXIT':
        #message = raw_input("ENTER THE RFC NUMBER ")
        #message1=raw_input("ENTER THE TITLE COMPLETE NAME OF THE FILE")
        ...
        for file in os.listdir(filepath):
            if file==filename:
                ...
                rfc_list.insert(temp)
        i+=1
#        message2=raw_input(" Wolud you like to make another entry?\n1. Press Y to make another entry      2. Press N to quit\n\n")
#        if message2=='N':
#            break 
#        
#    rfc_list.display()
    ...
    while True:
        ...
        if 'RFC_DOWNLOAD_REQUEST' in client_data:
            ...
            if found == 0:
                ...
                break                                           ###should send negative acknowledgment
            else:
